...
**2026-07-25 repo-maintenance pass (17th)** — scheduled, same six-point scope
...
was already green** at head `792eab0` (the 16th pass's own merge, CI run
30023130215, success) — no break to fix, and zero commits had landed since
that pass (this one started 3 days later). Five parallel independent audits
(not just re-reading the 16th pass's self-report) found two real, safe
mechanical fixes, one doc-bloat regression, one FLAG-ONLY doc gap, and
otherwise reconfirmed passes 10-16 clean:
1. **`apps/api/src/app.ts` lazy-imported `auth/audit.ts`/`auth/ratelimit.ts`
   22×** even though neither module imports `auth/tokens.ts` (the actual
   reason imports are lazy there — its module-level `AUTH_*` env-guard
   throws would break `api.test.ts`, which runs with no `AUTH_*` env set).
   `audit.ts`/`ratelimit.ts` only depend on `db.ts`, already statically
   imported — so they carried none of that risk. Made both static imports at
   the top of `app.ts` and deleted the 22 inline `await import(...)` calls;
   updated the header comment to explain why `tokens.ts` alone stays lazy.
2. **`apps/cli/.../Main.kt`'s `authedGet`/`authedDelete`/`authedPut`** were
   three ~14-line functions differing only by HTTP method and whether a body
   was threaded through — collapsed into one `authedCall(..., method, path,
   requestBody = null)` built directly on the existing `httpStatus` primitive
   (mirrors how `postStatus`/`putStatus`/`getStatus`/`deleteStatus` were
   collapsed into `httpStatus` itself by an earlier pass). `deleteStatus`
   became unused as a result and was removed; the six call sites were updated
   in place. Both fixes verified by careful manual diff review only (no
   npm/Gradle registry access in this sandbox — same constraint every prior
   code-touching pass hit) — will be confirmed by this PR's own CI.
3. **`backlog/now.md` itself had re-grown past its own stated pruning
   policy again** — the 16th pass fixed the 12th-vs-15th stacking but never
   moved its own 15th-pass paragraph out when it added the 16th-pass one,
   recreating the identical bug class right after fixing it. Moved both the
   2026-07-21 (15th) and 2026-07-22 (16th) paragraphs to `now-history.md`
   verbatim; this file keeps only this pass's entry going forward, per its
   own stated policy.
4. **FLAG-ONLY, not fixed this pass:** the CLI/skill-doc audit found a real,
   code-verified third 403 source for `dayfold push --hub` re-authoring an
   *existing* hub — the ADR-0030 §6 author/allow-list gate
   (`apps/api/src/app.ts` hub PUT route, ~line 735) fires for any non-legacy
   caller who isn't the hub's original author or an allow-listed
   participant/co-owner, **independent of scope and of the per-hub-role gate**
   that `Help.kt`/`cli.md` already document — and isn't exempted for
   CLI/device credentials the way the block-delete author-gate is. Neither
   `push --help` nor `cli.md`'s per-hub-role section mentions it, so an agent
   hitting this 403 would rule out the wrong cause. Left unfixed pending
   confirmation this is actually reachable under the single-approving-owner
   flow the curator skill assumes (not just a wording nitpick — worth a
   maintainer sign-off on the exact phrasing before it goes in `Help.kt`).
No CHANGELOG entry needed (fixes 1-3 are internal/doc-accuracy, not
product/API/feature changes; nothing shipped this pass). Values/privacy and
