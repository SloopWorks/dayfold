# Operator Inbox — history

Append-only archive of answered/resolved/superseded/closed entries moved out
of `backlog/operator-inbox.md` (2026-07-16 repo-maintenance pass) to keep that
file short for the start-of-session routine — same reason `backlog/now.md`
split into `now-history.md` on 2026-07-03 and `backlog/next.md` split into
`next-history.md`. **Not loaded by default** — read `operator-inbox.md`
first; come here only when you need the resolution narrative behind a past
decision. Entries are in the order they were written (newest-first), each
self-dated; a fully-resolved item's original ask is kept directly below its
resolution note, exactly as it was before the split.

Kept live in `operator-inbox.md` (not archived here) because they were still
open, `[pending-ratify]`, or had an operator-only remainder not confirmed
done from the repo alone: INB-32, INB-30, INB-27, INB-23 (G1–G4 mobile-release
setup gates), INB-19 (Maven Central publish + Homebrew symlink fix), INB-15
(reduxkotlin 1.0 feedback), INB-3.
...
- **INB-37 · RESOLVED 2026-08-08 — accepted as drafted, in session.** The operator
  ...
  subject family-wide with the byline as the only signal; and `label`/`sublabel`/`note`
  are plaintext at M0 under the block-payload posture. ADR 0064 flipped Proposed →
  Accepted and the build plan's Task 0 gate is cleared, so Phase A may start. The two
  carve-outs stand: the corrections / "fix it" channel is deferred to its own ADR
  (blocked on ADR 0062's run receipt), and the adults-only posture is untouched — ADR
  0064 §4 performs no role check because ADR 0004 makes every account holder an adult.
  Original ask below.
...
## Answered (bootstrap era, 2026-06-18)
