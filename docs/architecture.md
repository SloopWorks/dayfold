...
| Background refresh | `:client` `BackgroundRefresh.kt` + `AndroidBackgroundRefresh.kt` / `IosBackgroundNotify.kt` | Kotlin commonMain + WorkManager (Android) / BGAppRefreshTask (iOS) | ADR 0020 R3 — periodic bounded pass: drain `/sync` headlessly, then reconcile geofences + exact schedules, so the next open is already fresh; if the app's own runtime is already alive in-process it delegates the sync to it instead (avoids racing refresh-token rotation, which would sign the user out). **Live on Android only.** iOS's task is scheduled and cancellation-safe but has no configured API base yet (`backlog/next.md`'s open iOS sync-config gap), so every wake currently just reconciles local reminders and skips the network pull. Best-effort, OS-throttled either way; never a guaranteed cadence |
...
| Debug drawer | `third_party/debugdrawer` pinned to `SloopWorks/debugdrawer`: `debugdrawer`, `debugdrawer-noop`, `debugdrawer-redux`, `debugdrawer-swip` | Compose-MP composite build behind stable Maven coordinates | In-app devtools bubble/drawer (action log, redux state inspector); `-noop` is the release no-op while the real drawer and adapters are `debugImplementation`-only. `-swip` (ADR 0057) adds a live, mask-by-default analytics timeline panel fed by the SWIP capture engine's ring buffer. CI checks that release resolves only the no-op project and contains no real drawer/adapters in DEX |
