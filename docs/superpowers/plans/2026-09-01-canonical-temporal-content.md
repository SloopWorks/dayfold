...
> **Status:** implementation is blocked on operator acceptance of Proposed ADR
> 0067. This plan may be reviewed and revised now; Task 1 and later must not be
> built until acceptance.
...
- `apps/api/migrations/0021_temporal_content.sql` (new)
...
- `apps/client/src/commonMain/sqldelight/com/sloopworks/dayfold/client/db/migrations/16.sqm`
  (provisional: raw temporal/version/outbox capability; confirm next migration)
- `apps/client/src/commonMain/sqldelight/com/sloopworks/dayfold/client/db/migrations/17.sqm`
  (provisional: Calendar binding identity rebuild; confirm next migration)
- `apps/client/src/commonMain/sqldelight/com/sloopworks/dayfold/client/db/migrations/18.sqm`
  (provisional: desired notification schedule registry; confirm next migration)
...
- [ ] Add a mandatory pre-compose ledger with columns:
...
- [ ] Add a binding prompt-injection rule: email/calendar/file/note contents are
...
- [ ] Require one row per material claim. A range is one row with start/end; two
  distinct events are two rows.
- [ ] Define dispositions for non-operational cases: `reference`, typed source
  metadata, lifecycle, audit, or not-a-temporal-claim. A disposition cannot erase
  a material value from structured data.
- [ ] Resolve relative phrases against the source's own reference instant and
  relevant zone. Use authoring time only when the phrase refers to it explicitly.
  Missing/conflicting bases block.
- [ ] A tentative occurrence represents one stated tentative value. Competing
  alternatives for one fact block and require review; never turn alternatives
  into apparent event plurality.
- [ ] Unbounded recurrence and material month/year-only claims block V1. A finite
  expansion is allowed only when prose states the same finite horizon.
- [ ] Render the operator proposal from the ledger: entire resource JSON plus a
  concise temporal coverage table and any review issues.
- [ ] End the proposal turn after showing full JSON, facts, ACLs, base versions,
  ...
  trigger.
- [ ] Store local ledgers owner-readable, outside repo/CI/build artifacts, and
  delete after verified apply unless the operator explicitly retains one.
- [ ] Add fixtures for: one date, multiple same-day times, cross-midnight range,
...
- [ ] Define a size-bounded versioned local bundle containing resources, current
...
- [ ] Apply dry-run checks exact ledger path/value correspondence, duplicate mappings,
  missing mappings, temporal syntax, timezone/offset agreement, end ordering,
  and trigger references.
- [ ] Add conservative, linear-time local date/time mention detection for
  ...
  references.
- [ ] Default human/machine output includes issue code, structural path, and count,
  but not body excerpts or semantic IDs. An explicit local detail flag may show
  authorized resource IDs for repair.
- [ ] `audit` pulls only already-authorized Dayfold content, runs the same local
  coverage classifier, and writes nothing.
- [ ] Exit codes: `0` clean, `1` validation/coverage failure, `2` invocation/read
  failure. Curator apply always requires read access to every target; it never
  reports a write-only bundle as verified.
- [ ] There is no general bypass. Closed review-disposition codes require a later
...
- [ ] State explicitly that dry-run is diagnostic and does not authorize a later
  changed file. Task 5 always revalidates the in-memory bundle before writing.
- [ ] Bound bundle bytes, strings, resources, and audit pages before parsing;
  partial audits return an explicit incomplete status.
- [ ] Unit-test false-positive boundaries: years in names, version numbers, phone
...
- [ ] Add `$defs.TemporalOccurrence` and `$defs.TemporalFacet` with closed objects,
  enums, patterns, lengths, and array bounds.
- [ ] Add optional `temporal` to Block and BriefingCard.
- [ ] Add the closed `fact_ref` trigger union. A fact-ref variant carries only
...
- [ ] Define the exact reserved fact-key grammar for every carrier in the
  capability table; API, CLI, and client consume the same valid/invalid corpus
  under `specs/domain-model/examples/temporal-v1/`.
- [ ] Correct existing schema description drift: `$defs.timestamp` is date-time
  ...
  instead.
- [ ] Keep Hub `start_at`/`end_at` date-time-only. Remove/dead-code any client path
...
- [ ] Update `codegen.mjs` definition ordering/ref resolution: nested `$ref`s
  currently collapse to `z.any()`. Prove `TemporalFacetSchema`,
  `BlockSchema.temporal`, and `TriggerSchema.when` contain no `any`.
- [ ] Regenerate TypeScript Zod and Kotlin generated types with the pinned codegen.
- [ ] Inspect generated names/nullability and ensure the facet is not a generic
  map.
- [ ] Record the E2EE compatibility constraint from Proposed ADR 0015: schedule
...
- [ ] Update template/reference docs from the schema, not by copying a divergent
  hand-written shape.
...
- [ ] Add forward-only migration `0021_temporal_content.sql`:
...
- [ ] Implement tri-state update semantics on card/block routes: absent preserves,
...
- [ ] Preserve card visibility/audience on omission, matching hub PUT. Add card
  and hub `If-Match` support (sections/blocks already accept it); temporal
  repair/apply always sends reviewed ACL + base version.
- [ ] Upgraded full-replacement writers explicitly send temporal object/null.
  ...
  window.
- [ ] Ensure every list/tree/sync projection includes the columns by construction;
  add explicit round-trip tests to prevent a repeat of the accepted-but-dropped
  Hub.timeline bug.
- [ ] Parse with generated Zod and add one API cross-field validator for
  ...
  recognition.
- [ ] Enforce route-specific request-byte limits before `c.req.json()` allocation;
  string/array/occurrence limits then apply before date parsing.
- [ ] Validate same-item references against the effective stored/request value
  inside the same transaction. Cover concurrent omit/replace/clear and stale
  updates, not only sequential requests.
- [ ] Implement same-item fact-reference validation for plaintext M0 only. Treat
  the ADR 0015 split as a future acceptance constraint, not conditional V1 code.
- [ ] Map every expected failure to stable 422 paths/codes without throwing. Never
  echo parser-library values or PostgreSQL message/detail/row/parameters.
- [ ] Sanitize unexpected errors before SWIP/Sentry (prefer a content-safe reported
...
- [ ] Keep tenant visibility/auth unchanged. `temporal` inherits the parent row's
  family, hub visibility, audience, and author/write gates.
- [ ] Add malformed-size tests and ensure validation is linear in the bounded
  occurrence count.
...
- [ ] Reuse generated temporal types for shape and keep one CLI authoring validator
  for coverage, relative-time bases, zone/offset agreement, and the cross-field
  constraints required before a server call.
- [ ] Run deterministic temporal validation on every relevant `push`, independent
...
- [ ] Curator writes go through `content apply`, which verifies and writes the same
...
- [ ] Wire `dayfold content apply [--dry-run] <bundle.json>` as the only bundle
  verifier/write path. Real apply repeats the same in-memory validation.
- [ ] Treat apply as stateless and resumable, not atomic: require read access,
...
- [ ] On 200, compare canonical selected fields directly—no hash/digest. Ignore
...
- [ ] After all writes, pull the affected hub/card with the already read-authorized
...
- [ ] Verify ACL and version as well as temporal facts. A returned/pulled resource
  that widens or changes visibility/audience fails.
- [ ] For Hubs, read the existing audience/participant endpoint within the current
  credential scope; the tree response alone cannot prove the allow-list or roles.
- [ ] Print a final content-minimal receipt: resource counts, occurrence counts,
  fact-ref counts, review-issue counts, and `round_trip=verified`. Do not print,
  persist, transmit, or telemeter a schedule hash.
- [ ] Keep source phrases/labels out of logs, shell history guidance, and error
  telemetry.
...
- [ ] Add raw `JsonObject`/JSON-text `temporal` fields to `Card` and `HubBlock`.
...
- [ ] Add nullable `temporal TEXT` to `card` and `hub_block`, plus Card version/ACL
  fields needed for safe replacement, in provisional migration `16.sqm` (confirm
  the actual next sequence at implementation time).
- [ ] Thread JSON through every `ContentStore.applyDelta`, row projection, fake
  backend, full-resync, and cache test.
- [ ] Preserve unknown future roles/statuses as non-eligible rather than failing an
  entire sync page.
- [ ] Make the `CLIENT_SCHEMA_VERSION` bump mandatory when temporal-aware readers
...
- [ ] Member/checklist/Calendar field writes omit `temporal`; server tri-state
...
- [ ] Preserve every whole-resource field in caches/egress, including raw unknown
...
- [ ] Stamp each outbox row with the writer schema/capability at creation. Request
...
- [ ] Preserve raw unknown trigger members on upgraded clients. An old client can
...
- [ ] Parse date-only as civil interval and instants separately; never coerce an
  all-day date at UTC/device midnight.
- [ ] Hub normalization accepts only the timed values its schema/storage can
  preserve; it does not route Hub strings through `AllDay` in V1.
- [ ] Treat stored timed instant as authoritative. Invalid syntax remains prose-
...
- [ ] Implement the exact carrier/capability/fact-key table.
- [ ] Never merge explicit IDs by value. Reject double-authoring at CLI; normalizer
  preserves both if malformed legacy data still reaches it.
- [ ] Treat legacy `when.at` as a Calendar compatibility fallback only when the
...
- [ ] Expose consumer-specific queries (`forTrigger`, `calendarEligible`,
  `timelineEligible`) rather than letting each consumer reimplement predicates.
- [ ] Resolve and write back by the closed `fact_ref` grammar, including checklist
  item ID or typed payload field. No separate locator identity and no title/array-
  position identity.
- [ ] Keep clock/zone injected; no wall-clock reads in the pure model.
...
- [ ] Keep hub start/end and milestone compatibility through the normalizer.
- [ ] Emit one candidate per **capability-eligible** confirmed
  event/deadline/window fact. Do not newly emit checklist/card payload dates that
  the matrix marks Calendar-ineligible.
- [ ] Give `DayfoldEventCandidate` distinct `entityRef`, existing `subjectRef`, and
...
- [ ] Rebuild/migrate device-local `calendar_binding` so entity + fact form the
...
- [ ] Thread `entityRef`/`factRef` and the derived `localFactKey` through Calendar
...
- [ ] Do not emit reference/tentative/cancelled occurrences.
- [ ] Preserve typed-field-only and on-device-only rules; never inspect body text.
- [ ] Timed candidate matching, fingerprints, and diffs compare parsed instants;
...
- [ ] Define all-day end translation explicitly for Android/iOS native APIs.
  Fix iOS's `start + 1.days` default: compute next civil date then its start so
  23/25-hour DST days remain one all-day event.
- [ ] Re-run duplicate matching with several occurrences sharing a title/time.
...
- [ ] Verify add-only handoff and Calendar Check read/write consent are unchanged;
  no fact causes an unreviewed native write.
- [ ] Stop Calendar import from materializing arbitrary external descriptions as
...
- [ ] Route derived-timeline input through the resource-local normalizer and a
  small hub-tree collector; do not introduce a global cross-family index.
- [ ] Preserve authored `Hub.timeline` precedence from ADR 0045.
- [ ] For derived fallback only, emit separate confirmed event/deadline/window
  facts that the capability table marks eligible, with stable source tags.
- [ ] Do not emit reference/tentative/cancelled occurrences.
- [ ] Do not value-deduplicate legitimate simultaneous facts. Reject/mark malformed
  double-authoring using source identity, never timestamp/title equality.
- [ ] Extend the derived presenter input with a client-only/transient
...
- [ ] Keep render-only/no-notification posture.
- [ ] Update the ADR 0046 compatibility test: legacy multiple-date blocks keep
...
- [ ] Extend `TriggerWhen` with `factRef` and the closed fact-ref trigger variant.
- [ ] Resolve through the same parent resource's normalized facts.
- [ ] Keep `eventAt` separate from offset-adjusted `effectiveAt` exactly as current
  Now selection does.
- [ ] Reject/ignore dangling, all-day, reference, tentative, cancelled, and invalid
  facts. Malformed `alert_offset` is an authoring/API error, not zero offset.
- [ ] Continue reading legacy `when.at`; new writes use the closed ref variant.
- [ ] Enforce at most one fact-ref trigger per parent in V1. Multiple behavior is a
  later ADR, not silently selected/dropped.
- [ ] Do not emit fact refs until the old-writer gate from Task 6 is closed. Add
...
- [ ] After that gate closes, change Calendar import's timed milestone trigger
  from the compatibility `when.at` copy to `fact_ref=payload:milestone`.
- [ ] Use the derived entity-plus-fact `localFactKey` for notification ownership/
...
- [ ] Add a device-local desired-schedule registry keyed by `localFactKey`. Diff it
...
- [ ] Ensure `expires_at` remains lifecycle authority, not inferred from event end.
- [ ] Exact local schedules use only explicitly triggered occurrences; temporal
...
- [ ] Before repair, ship/test card ACL preserve-on-omit, `If-Match`, and direct
...
```bash
cd apps/api
npx vitest run test/temporal-validation.test.ts test/temporal-persist.test.ts
npm test
```

Live-Postgres tests apply migrations through `0021`; never modify an already
applied migration.
