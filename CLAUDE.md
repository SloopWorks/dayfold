...
stage tracker — read it first. As of 2026-07-11: **M0 render prototype is
...
Hub Timeline (authored + on-device-derived, ADR 0045/0046), Now-derived
priority surfacing + Android background notifications (ADR 0043/0044),
DB-first cold-start routing (ADR 0052), and per-hub participation roles +
profile avatars + in-app hub People management (ADR 0053; **the CLI still
only grants read-only Viewer access — Contributor/Co-owner is app-UI-only**).
A debug-build-only bug reporter (ADR 0054, `apps/swip-wiring`) ships zero
bytes in release builds.
...
| `designs/` | Hi-fi UI/UX mockups (ADR 0008 design-first gate) | Source of truth once operator-signed-off |
