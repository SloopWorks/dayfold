...
**H1–H4 are confirmed** — not just viable but industry-convergent: the
newest generation of client architectures (LiveStore, Replicache/Zero,
Linear's sync engine) independently re-derived "named replayable actions +
derived state + ports at the edge" as the way to build offline-first,
multi-device apps [fact:https://dev.docs.livestore.dev/evaluation/event-sourcing/,
https://doc.replicache.dev/concepts/how-it-works,
https://github.com/wzhudev/reverse-linear-sync-engine]. H2 is confirmed
with a specific answer: **five loading primitives + one read-state type**
(§4.2).

**H5 is refuted in its literal form, on one specific axis** — and every
mature practitioner community independently drew the same line: *durable
intent* belongs in the store; *high-frequency ephemeral signals*
(keystrokes, scroll, drag frames, live sensor/location) must stay out and
enter only as settled results (§4.3, §5.1). Twitter Lite measured the cost
of ignoring this: moving per-keystroke composer state out of the global
store "reduced overhead by over 50%"
[fact:https://medium.com/@paularmstrong/twitter-lite-and-high-performance-react-progressive-web-apps-at-scale-d28a00e780a3].

**H6's answer:** no documented healthy app runs *one flat typed* store past
roughly the low hundreds of action types [estimate — extensive search found
no counter-example; Redux's own FAQ's largest cited healthy deployment is
~500 string-typed actions, and the best-documented typed case (a TCA app
built by 8 teams) broke the IDE and required stack-size increases
[fact:https://redux.js.org/faq/performance,
https://rodschmidt.com/posts/composable-architecture-experience/]]. But the
...
- **Swift TCA** is the best-documented *typed* single-store failure case:
  ...
  Their reference app (isowords, ~50k LOC) scales by **~90 SPM modules**
  with per-feature action enums [fact:https://github.com/pointfreeco/isowords].
...
auth flows = Mutations. **Nothing in the app required a sixth primitive**
— dayfold is itself a confirming instance [fact:memo 4].
...
2. **Intent, not intermediate deltas.** An event log's distinguishing
   ...
   purpose-built columnar encoding — full-history JSON action logs grow
   ~50× faster than the state they describe [estimate, from
...
### 4.4 H4 confirmed: the hexagonal mapping is exact, not analogical

Cockburn's ports-and-adapters, Bernhardt's functional-core/imperative-shell,
and Elm's managed effects (Cmd/Sub) are the same design at three levels of
strictness [fact:https://alistair.cockburn.us/hexagonal-architecture,
https://www.destroyallsoftware.com/talks/boundaries,
https://guide.elm-lang.org/effects/]. In redux terms: **reducers +
...
- **iOS/Swift**: the type-checker's known super-linear behavior on large
  enums with associated values + exhaustive switches is TCA's documented
  compile-time pain; isowords' ~90-module split is the fix in the wild
...
- **Web/Wasm**: many distinct action *shapes* make generic
  ...
  — the flat-standard-action shape (`{type, payload, meta}`) is
  monomorphic by construction [fact:memo 2 §3]. Kotlin/Wasm binary size
...
- **The central Action type is the worst build-cache key in the
  ...
  documented pain point at scale; a 10⁴-subtype sealed hierarchy puts
  enumeration cost on the *interactive* path (every completion popup,
  every exhaustiveness inspection)
  [fact:https://youtrack.jetbrains.com/issue/KTIJ-8163].
...
- The moment actions are **persisted or transmitted** (outbox, sync,
  ...
  persisted in SQLite and replayed after app upgrades [fact:memo 4 §5.4].
...
| 4 | O(N) `when` dispatch + 64KB method limit | Hierarchical action space: `sealed interface FeatureAction : Action` per feature; root reducer matches the ~30 feature groups, delegates to per-feature reducers (O(features + feature-actions)) | isowords module tree; RTK slices |
| 5 | Whole-app recompile on any new action | Same sharding, but as *module* boundaries: per-feature action types live in the feature module; the root composes interfaces. Codegen'd action modules as rarely-changing leaf deps | isowords (~90 modules); Kotlin classpath-snapshot IC |
...
- **Verification collapses in dimensionality.** Testing UI = testing
  ...
  with the five ports [assumption, grounded §4.2/§4.3].
- **Maximal code sharing** — the core is platform-free by construction;
  platform surface = adapters + rendering (dayfold: ~1.2k platform LOC of
  15.8k main).
...
- **R1 (cheap, soon).** Pull the already-specified selector lever: narrow
  `selectorState`/`fieldState` bindings for the hot screens + the
  spec'd-but-unbuilt recomposition-count CI guard
  (`specs/prototype/08-mobile-client.md`), before Hubs/Now content volume
  grows. This is the top predicted first regression (§5.2).
- **R2 (threshold trigger).** When `Action` approaches ~150–200 subtypes
  or `Reducer.kt` ~400 lines, shard: per-feature `sealed interface
  XxxAction : Action` + per-feature reducer functions, root reducer
  matching feature groups. File-level first; module-level only if build
  times demand (solo-dev repo — team-coupling pressure doesn't apply).
...
- **R5 (contract hygiene).** The outbox already persists action-shaped
  ...
  upcast-on-read now, while there is one op type and zero migration debt
  (§5.6).
...
- **Round 1 (correctness):** *recorded below after review.*
- **Round 2 (optimization/simplification):** *recorded below after
  review.*
