...
3. **ADR index numbering 0066–0070 and whether ADR 0071's number is safe.**
   Safe, and the "reservation" premise doesn't exist in the committed packet.
   `adr/decisions-index.md` jumps directly from row 0065 to row 0071
   (`adr/decisions-index.md:81-82`); no `adr/0066-*.md` … `adr/0070-*.md` file
   exists (`ls adr/`); no committed file (grepped across `.md` repo-wide)
   claims 0066–0070 are reserved for anything. `adr/0071-self-managed-claude-bridge-v0.1.md`
   is the sole file using that number and the index row matches its filename
   and title exactly. The five-number gap is simply unused headroom, not a
...
`packages/`, or `specs/domain-model/` in the first place. All five findings
matter starting at WP1/Phase C-D, once implementation against
`packages/routine-schema/`, `apps/cli`, and ADR 0071/0061/0062 text actually
begins.
...
3. All nine "Operator decisions" already enumerated in
   ...
   fresh session doesn't mistake the findings above (F1-F5, all
   documentation/sequencing corrections) for new items on that list.
