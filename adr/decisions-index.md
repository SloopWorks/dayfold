...
| 0058 | Client Runtime and Effect Ownership | Accepted 2026-07-14 (operator accepted in-session) | Narrowly supersedes ADR 0013's middleware-only effect-origin rule with explicit runtime-owned commands/engines while retaining pure reducers, off-main effects, UI-thread notifications, render isolation, state-keyed lifecycle, and tenant-safe structured ownership. |
| — | *(0059 reserved — API error-pillar ADR, in-flight PR #336; not on this branch)* | — | — |
| 0060 | Client Crash / Error Reporting — Debug-Only Android, SWIP Error Pillar → Sentry (KMP project) + PostHog | Proposed 2026-07-15 (agent-drafted; accept on merge) | Turns on the SWIP `errors` pillar for the Android dogfood build: fatal crashes mirrored to PostHog via a marker file, handled `record()`/`wtf()` id-joined to Sentry, independent org/project constant as the wrong-project guard, `Application`-hoisted init, debug-only consent honest only because it's the operator's own device. |
