...
class TimelineActionsTest {
  @Test fun `maps each kind`() {
    ...
    assertEquals(CardAction.OpenHub("h1", HubArrival(HubArrivalLevel.BLOCK, "b2", HubArrivalSource.BRIEFING)), Attachment("open","List",ref=AttachmentRef("h1",blockId="b2")).toCardAction())
    ...
    assertEquals(CardAction.OpenHub("h1", HubArrival(HubArrivalLevel.SECTION, "sec-money", HubArrivalSource.BRIEFING)), Attachment("open","Money & Billing",ref=AttachmentRef("h1",sectionId="sec-money")).toCardAction())
    // blockId wins when both are present (a specific block is more precise than its section).
    assertEquals(CardAction.OpenHub("h1", HubArrival(HubArrivalLevel.BLOCK, "b2", HubArrivalSource.BRIEFING)), Attachment("open","x",ref=AttachmentRef("h1",sectionId="s1",blockId="b2")).toCardAction())
  }
