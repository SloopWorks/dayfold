...
fun rootReducer(state: AppState, action: Any): AppState = when (action) {
  ...
  is SignInRequested, is SignInFailed, is SessionRotated, is CreateFamilyRequested, is AuthOpFailed, is SignOutRequested, is DeleteAccountRequested, is DeleteAccountFailed, is InviteLinkStashed, is InviteLinkConsumed -> reduceSession(state, action)
...
private fun reduceRoutedFeatureWithFamilyTransition(state: AppState, action: Any): AppState {
  ...
  val familyScoped = if (familyChanged) updated.copy(
    ...
    now = NowState(),
    hubs = HubState(),
    // ADR 0064 — rules are family-scoped synced content, so a family switch invalidates them
    // exactly like cards and hubs. Carrying them across would let one family's mutes suppress
    // another's feed until the first sync landed.
...
private fun signedOutState(state: AppState) = AppState(
  ...
  calendar = state.calendar.copy(check = CalendarCheckState(), importState = ImportProposalState.None),
