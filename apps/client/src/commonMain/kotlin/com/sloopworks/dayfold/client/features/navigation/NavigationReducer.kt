...
fun reduceNavigation(state: AppState, action: Any): AppState = when (action) {
  ...
  is OpenCalendarSettings -> state.copy(navigation = state.navigation.copy(route = Route.CalendarSettings))
  is CloseCalendarSettings -> state.copy(navigation = state.navigation.copy(route = Route.Account))
  is OpenCalendarReview -> state.copy(navigation = state.navigation.copy(route = Route.CalendarReview))
  is CloseCalendarReview -> state.copy(navigation = state.navigation.copy(route = Route.Feed))
  is OpenCalendarImport -> state.copy(navigation = state.navigation.copy(route = Route.CalendarImport))
  is CloseCalendarImport -> state.copy(navigation = state.navigation.copy(route = Route.CalendarReview))
