...
// WI-461 — Calendar routes join the same list: ADR 0063 forbids raw calendar state (events,
// ids, account names, match decisions) from ever entering a journal, and even "the family
// opened Calendar Check" is a weak signal about a household's calendar-sync posture worth
...
private fun recordedRoute(route: Route): String =
  if (
    ...
    route == Route.SmartContent ||
    route == Route.CalendarSettings ||
    route == Route.CalendarReview ||
    route == Route.CalendarImport
  ) Route.Account.name else route.name
