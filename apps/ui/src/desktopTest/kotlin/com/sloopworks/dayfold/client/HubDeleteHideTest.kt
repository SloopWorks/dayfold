...
class HubDeleteHideTest {
  ...
  @Test fun authorCanOpenTheDeleteSheetAndConfirm() = androidx.compose.ui.test.runComposeUiTest {
    ...
    val state = AppState(session = me, hubs = HubState(currentHubId = "h1", currentHubTree = treeWith(textBlock("b1", author = "usr_mom"))))
  ...
  @Test fun nonAuthorSeesNoDeleteOption() = androidx.compose.ui.test.runComposeUiTest {
    val state = AppState(session = me, hubs = HubState(currentHubId = "h1", currentHubTree = treeWith(textBlock("b1", author = "usr_sam"))))
  ...
  @Test fun aBlockWithNoAuthorIsNotDeletable() = androidx.compose.ui.test.runComposeUiTest {
    val state = AppState(session = me, hubs = HubState(currentHubId = "h1", currentHubTree = treeWith(textBlock("b1", author = null))))
    setContent { MaterialTheme { HubDetailScreen(state) } }
    onNodeWithContentDescription("More options").performClick()
