...
class HubChecklistToggleTest {
  ...
  @Test fun `a done item shows the toggler's first name, not the raw userId`() = runComposeUiTest {
    val state = AppState(
      ...
      familyAdmin = FamilyAdminState(members = listOf(FamilyMember(uid = "u_pat", displayName = "Patrick Jackson"))),
  ...
  @Test fun `a done item by an unknown member falls back to a family member`() = runComposeUiTest {
    val state = AppState(
      session = SessionState(session = Session("a", "r", userId = "u_me")), familyAdmin = FamilyAdminState(members = emptyList()),
