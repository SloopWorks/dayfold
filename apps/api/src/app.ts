...
// tokens.ts imports stay lazy (dynamic, at each call site below) so that api.test.ts
// (no AUTH_* env) can still load app.ts without triggering tokens.ts's module-level
// env-guard throws. audit.ts/ratelimit.ts only depend on db.ts, already static above,
// so they carry none of that risk and are imported normally.
