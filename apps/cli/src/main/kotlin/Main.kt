 ...
 *  `getStatus` below were four near-identical ~8-line functions (a fourth,
 *  `deleteStatus`, was later removed once `authedCall` made it unused) differing
 *  only by method — collapsed by a repo-maintenance pass. */
 ...
 * guidance on any failure. Shared by authedCall's/push's 401-retry path (was
 * inlined three times, byte-for-byte, before this extraction).
 */
...
/**
 * Authed HTTP call with one transparent refresh on 401 (device creds only; the
 * legacy env path has no refresh). Returns Pair<statusCode, body>. Collapses what
 * were three near-identical authedGet/authedDelete/authedPut functions, differing
 * only by method and whether a body was threaded through (repo-maintenance pass).
 */
private fun authedCall(
  store: Credentials?, keychain: SecretStore?,
  api: String, token: String, refreshable: Creds?, method: String, path: String, requestBody: String? = null,
): Pair<Int, String> {
